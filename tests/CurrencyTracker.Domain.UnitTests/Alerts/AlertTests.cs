...
public sealed class AlertTests
{
    ...
    private static readonly DateOnly TestAsOfDate = new(2025, 6, 1);
    ...
    {
        var result = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 1.05m, TestFiredAt);

        result.IsSuccess.Should().BeTrue();
        result.Value.RuleId.Should().Be(TestRuleId);
        result.Value.AsOfDate.Should().Be(TestAsOfDate);
    ...
    {
        var result = Alert.Create(Guid.Empty, TestAsOfDate, 1.00m, 1.05m, TestFiredAt);
    ...
    [Fact]
    public void Create_default_asOfDate_returns_failure_with_ALERT_AS_OF_DATE_REQUIRED()
    {
        var result = Alert.Create(TestRuleId, default, 1.00m, 1.05m, TestFiredAt);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("ALERT_AS_OF_DATE_REQUIRED");
    }

    [Fact]
    public void Create_non_positive_previousRate_returns_failure_with_ALERT_PREVIOUS_RATE_NONPOSITIVE()
    {
        var result = Alert.Create(TestRuleId, TestAsOfDate, 0m, 1.05m, TestFiredAt);
    ...
    {
        var result = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, -0.01m, TestFiredAt);
    ...
    {
        var result = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 1.05m, default);
    ...
    public void Create_computes_observed_change_percent_as_absolute_for_decreasing_rate()
    {
        ...
        var result = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 0.90m, TestFiredAt);
    ...
    {
        var alert = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 1.05m, TestFiredAt).Value;
    ...
    {
        var a = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 1.05m, TestFiredAt).Value;
        var b = Alert.Create(TestRuleId, TestAsOfDate, 1.00m, 1.05m, TestFiredAt).Value;
